-- Add price history columns to deliveries table
ALTER TABLE deliveries ADD COLUMN IF NOT EXISTS original_rate NUMERIC;
ALTER TABLE deliveries ADD COLUMN IF NOT EXISTS is_price_synced BOOLEAN DEFAULT false;

-- Update the record_delivery_atomic_item function to handle original_rate
CREATE OR REPLACE FUNCTION record_delivery_atomic_item(
    ...
    p_delivery_type TEXT DEFAULT 'short', -- 'short' or 'partial'
    p_original_rate NUMERIC DEFAULT NULL  -- New parameter for price history
) RETURNS UUID AS $$
DECLARE
    v_po RECORD;
    v_item JSONB;
    ...
    v_prev_delivered_qty NUMERIC := 0;
    v_total_delivered_now NUMERIC := 0;
    v_item_status TEXT;
BEGIN
    ...
    v_prev_delivered_qty := COALESCE((v_item->>'delivered_quantity')::NUMERIC, 0);
    ...
    v_total_delivered_now := v_prev_delivered_qty + p_received_qty;
    
    -- Delivered amount logic for partials: cap at remaining
    IF v_ordered_qty - v_prev_delivered_qty > 0 THEN
        v_delivered_amount := LEAST(p_received_qty, v_ordered_qty - v_prev_delivered_qty) * v_rate_per_liter;
    ELSE
        -- Over-delivery on already fully delivered (shouldn't happen but safe)
        v_delivered_amount := p_received_qty * v_rate_per_liter;
    END IF;
    
    v_remaining_qty := v_ordered_qty - v_total_delivered_now;
    
    -- Only create hold if it's a short delivery AND there is a remaining quantity
    IF p_delivery_type = 'short' AND v_remaining_qty > 0 THEN
    ...
    IF v_total_delivered_now >= v_ordered_qty OR p_delivery_type = 'short' THEN
        v_item_status := 'delivered';
    ELSE
        v_item_status := 'partially_delivered';
    END IF;
    ...
    INSERT INTO deliveries (
        ...
        COALESCE(p_original_rate, (v_item->>'original_rate')::NUMERIC),
        (p_original_rate IS NOT NULL OR v_item ? 'original_rate')
    ...
    v_items_array := jsonb_set(
        ...
        v_item || jsonb_build_object(
            'delivered_quantity', v_total_delivered_now,
    ...
    IF v_hold_amount > 0 AND p_delivery_type = 'short' THEN
