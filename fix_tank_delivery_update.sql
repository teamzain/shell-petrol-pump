-- =====================================================
-- FIX: Tank Stock Update during Delivery
...
DECLARE
    ...
    v_pkt_time TIME;
BEGIN
    -- Get Pakistan Time for movements
    v_pkt_time := (now() AT TIME ZONE 'Asia/Karachi')::time;
    ...
    -- 1. Get PO details and lock
    SELECT * INTO v_po FROM purchase_orders WHERE id = p_po_id FOR UPDATE;

    IF v_po.status = 'closed' OR v_po.status = 'cancelled' THEN
        RAISE EXCEPTION 'This PO is closed or cancelled.';
    END IF;
    ...
    INSERT INTO deliveries (
        ...
        v_ordered_qty, p_received_qty, GREATEST(0, v_ordered_qty - p_received_qty),
        v_rate_per_liter, (v_ordered_qty * v_rate_per_liter), 
        LEAST(p_received_qty, v_ordered_qty) * v_rate_per_liter,
        GREATEST(0, (v_ordered_qty - p_received_qty) * v_rate_per_liter),
    ...
    IF v_product_id IS NOT NULL THEN
        ...
        -- 4b. Update Tanks Stock (CRITICAL FIX HERE)
        IF p_tank_distribution IS NOT NULL AND jsonb_array_length(p_tank_distribution) > 0 THEN
            FOR v_tank_element IN SELECT * FROM jsonb_array_elements(p_tank_distribution) LOOP
                ...
                IF v_tank_qty > 0 THEN
                    -- More lenient update: Trust the ID provided by the UI which is already filtered by product
                    UPDATE tanks 
                    SET 
                    ...
                    -- Log to tank_stock_movements if exists (optional but good)
                END IF;
            END LOOP;
        END IF;
        ...
        INSERT INTO stock_movements (
            ...
            (p_delivery_date::text || ' ' || v_pkt_time::text)::timestamp AT TIME ZONE 'Asia/Karachi'
        );
    ...
    -- 5. Handle Hold
    IF (v_ordered_qty - p_received_qty) > 0 THEN
        INSERT INTO po_hold_records (
            ...
            p_po_id, v_delivery_id, (v_ordered_qty - p_received_qty), 
            (v_ordered_qty - p_received_qty) * v_rate_per_liter, 
            p_user_id, v_po.supplier_id, v_product_id, v_product_name, p_item_index
        );
    END IF;
