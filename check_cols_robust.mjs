...
async function check() {
    ...
    if (cols1 && cols1[0]) {
        console.log('TABLE: card_hold_records');
        Object.keys(cols1[0]).forEach(k => console.log('COL:', k));
    }
    if (cols2 && cols2[0]) {
        console.log('TABLE: daily_sales');
        Object.keys(cols2[0]).forEach(k => console.log('COL:', k));
    }
    console.log('--- END ---');
}
check();
